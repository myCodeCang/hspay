...
function powerConfig(args) {
	var defaults = { "width":150, "height":22, "maxLength":20, "minLength":6, "maskChar":"*", "backColor":"#FFFFFF", "textColor":"#0000FF", "borderColor":"#ffffff", "accepts":"\\w{0,}", "caption":"�ڸ�ͨ", "captionColor":"#FFFFFF", "captionFont":"", "captionSize":0, "captionBold":"true", "lang":"zh_CN"};
	for (var p in args)
...
function writePluginObject(oid, clsid, cfg) {
	if(cfg.classnm!=null&&cfg.classnm!=""){
...
function writeObject(oid, clsid, cfg) {
	if(cfg.classnm!=null&&cfg.classnm!=""){
		document.write('<object tabindex="2" id="' + oid + '" codebase="/newweb/setup/PowerEnterTFTPAY.CAB#version=2,3,3,0" classid="' + clsid
				+ '" class="' + cfg.classnm 
				+ '">');
	}
	else{
...
function showTFTSetup(passid, name){
	...
	tempObjHTML = '<a href="https://www.tftpay.com/newweb/script/'+name+'/setup/PowerEnterTFTPAY.exe">请安装安全输入控件</a>';
